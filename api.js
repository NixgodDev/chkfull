...
async function validarCartaoZeroAuth(cardData) {
    ...
    try {
        console.log('Enviando requisição para ZeroAuth:', zeroAuthUrl);
        ...
        console.log('Resposta do ZeroAuth:', response.data);
        return response.data;
    } catch (error) {
        console.error('Erro no ZeroAuth:', {
...
async function criarCobranca(cardData, valor) {
    ...
    try {
        console.log('Enviando requisição para criar cobrança:', CIELO_API_URL);
        ...
        console.error('Erro na API da Cielo:', {
            status: error.response ? error.response.status : 'No status',
            data: error.response ? error.response.data : 'No data',
            message: error.message
        });
...
app.post('/pagamento', async (req, res) => {
    ...
    if (!cardData || !cardData.cardNumber || !cardData.expirationDate || !cardData.securityCode) {
        console.error('Formato do cartão inválido:', cardData);
    ...
    if (!luhnCheck(cardData.cardNumber)) {
        console.error('Cartão inválido (Luhn):', cardData.cardNumber);
        return res.status(400).json({
            status: 'Erro',
    ...
    if (!zeroAuthResponse.Valid) {
        console.error('Cartão inválido (ZeroAuth):', zeroAuthResponse);
        return res.status(400).json({
            status: 'Erro',
